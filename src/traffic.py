...
def _fetch_intent_logs_from_cloudwatch_by_date(date_str: str) -> List[dict]:
    """
    Fetch intent logs from CloudWatch for a specific date (in PKT timezone).
    date_str should be in YYYY-MM-DD format.
    """
    client = get_cloudwatch_client()

    # Parse date and create PKT midnight-to-midnight range
    date = datetime.strptime(date_str, '%Y-%m-%d')
    range_start = datetime(date.year, date.month, date.day, 0, 0, 0, tzinfo=PKT)
    range_end = datetime(date.year, date.month, date.day, 23, 59, 59, tzinfo=PKT)

    # Cap end time to current time if date is today
    now_pkt = datetime.now(PKT)
    if range_end > now_pkt:
        range_end = now_pkt

    api_start_time_ms = int(range_start.astimezone(timezone.utc).timestamp() * 1000)
    api_end_time_ms = int(range_end.astimezone(timezone.utc).timestamp() * 1000)
    ...
    if iteration >= max_iterations:
        print(f"Warning: Reached max iterations ({max_iterations}) for traffic fetch, results may be incomplete")
...
def get_intent_traffic_data_by_date(date: str) -> Dict:
    """Get intent traffic data for a specific date (YYYY-MM-DD format, PKT timezone)."""
    ...
    # Ensure date is not in the future (PKT)
    now_pkt = datetime.now(PKT)
    if parsed_date.date() > now_pkt.date():
        raise HTTPException(status_code=400, detail="Cannot select a future date.")

    # Check cache first
    cache_key = generate_cache_key("traffic_date", date, None)
    cached_events = get_cached_logs(cache_key)
    ...
    if cached_events is not None:
        ...
        print(f"Cache MISS: {cache_key} - fetching from CloudWatch...")
        try:
            all_events = _fetch_intent_logs_from_cloudwatch_by_date(date)
            # Cache for past dates longer (they won't change), today shorter
            cache_hours = 24 if parsed_date.date() < now_pkt.date() else 1
            set_cached_logs(cache_key, all_events, cache_hours)
        except Exception as e:
            print(f"Traffic fetch error: {type(e).__name__}: {str(e)}")
            raise HTTPException(status_code=500, detail=f"Error fetching traffic data: {type(e).__name__}: {str(e)}")

    # Use hourly buckets for a full day (24 buckets)
    bucket_minutes = 60
    num_buckets = 24

    # Day start in PKT
    day_start_pkt = datetime(parsed_date.year, parsed_date.month, parsed_date.day, 0, 0, 0, tzinfo=PKT)
    ...
    for i in range(num_buckets):
        bucket_time = day_start_pkt + timedelta(hours=i)
    ...
    for event in all_events:
        ...
        if intent:
            event_time = datetime.fromtimestamp(timestamp / 1000, tz=timezone.utc)
            event_pkt = event_time.astimezone(PKT)
            bucket_index = min(event_pkt.hour, num_buckets - 1)

            bucket_data[bucket_index][intent] += 1
            totals[intent] += 1
